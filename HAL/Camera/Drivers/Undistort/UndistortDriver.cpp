...
namespace hal
{
      ...
      ) : m_Input(input)
  {
    ...
    CameraDriverInterface::SetDefaultProperties({
        {"file","","XML camera models file."}
        });
    if( !CameraDriverInterface::ParseUriProperties( uri.properties ) ){
      std::cerr << "UndistortDriver knows about the following properties:\n";
      CameraDriverInterface::PrintPropertyMap();
      return;
    }

    std::string filename = 
      ExpandTildePath( GetProperty<std::string>("file", "cameras.xml") );
    ...
    for(size_t ii=0; ii< num_cams; ++ii) {
      ...
      std::shared_ptr<calibu::CameraInterface<double>> 
        new_cam(new calibu::LinearCamera<double>(params_, size_));
