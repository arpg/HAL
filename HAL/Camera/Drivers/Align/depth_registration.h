...
protected:
  cv::Mat cameraMatrixColor, cameraMatrixDepth, rotation, translation,
    mapX, mapY;
...
public:
  ...
  virtual void depthToRGBResolution(const cv::Mat &registered,
                                    cv::Mat &upscaled) = 0;

  static DepthRegistration *New(const cv::Size &color, const cv::Size &depth,
                                const cv::Size &raw, const float zNear = 0.5f,
                                const float zFar = 12.0f,
                                const float zDist = 0.015f,
