...
int main( int argc, char* argv[] )
{
    ...
    hal::Camera cams;
    if(bHaveCam) {
	//printf("arg=%s, %s, %s, %s\n", sCam[0].c_str(), sCam[1].c_str(), sCam[2].c_str(), sCam[3].c_str());
    for(int ii=0;ii<NUM_CAM; ii++)
	    cams = hal::Camera(sCam);

        nNumChannels = cams.NumChannels();
        nImgWidth = cams.Width();
        nImgHeight = cams.Height();

        //std::cout << "- Opening camera with " << nNumChannels << " channel(s)." << std::endl;
        //for(size_t ii=0; ii<NUM_CAM; ++ii) {
        //    std::cout << "  " << cams[ii].Width(0) << "x" << cams[ii].Height(0) << std::endl;
        //}
    }
    ...
    //This line is a gui related call back, but is placed here so that when call back is registred LIDAR handler can upload data to buf.
    ...
    ///-------------------- WINDOW INIT
    int nWindowHeight, nWindowWidth;

    //Size of the window.
    if(bHaveLIDAR & bHaveIMU & bHaveCam)
    {
	nWindowWidth=1024;
	nWindowHeight=768;
    }
    ...
    {
	nWindowWidth = NUM_CAM*nImgWidth;
	nWindowHeight = nImgHeight *NUM_CAM;//* (bHaveIMU ? 3.0/2.0 : 1.0);
    }

    // Setup OpenGL Display (based on GLUT)
	pangolin::CreateWindowAndBind("All the Sensors, Yo!!",nWindowWidth,nWindowHeight);
	...
	pangolin::View& cameraView = pangolin::CreateDisplay();//.SetBounds(0.75,1.0,0.0,1.0);//.SetLayout(pangolin::LayoutEqualHorizontal);
	if(bHaveCam)
	{
	    for(size_t ii=0; ii < NUM_CAM; ++ii ) {
		cameraView.AddDisplay(pangolin::CreateDisplay().SetBounds(0.75,1.0,ii*0.25,(ii+1)*0.25));
	    }
	}

	if( bHaveIMU ) {
	    pangolin::View& imuView = pangolin::CreateDisplay();//.SetLayout(pangolin::LayoutEqualVertical);
	    imuView.AddDisplay( pangolin::CreatePlotter("Accel", &g_PlotLogAccel).SetBounds(0.5,0.75,0.75,1) );
	    imuView.AddDisplay( pangolin::CreatePlotter("Gryo", &g_PlotLogGryo).SetBounds(0.25,0.5,0.75,1) );
	    imuView.AddDisplay( pangolin::CreatePlotter("Mag", &g_PlotLogMag).SetBounds(0.0,0.25,0.75,1) );
	...
	{
	    d_cam.SetBounds(0.0, 0.75, 0.0, 0.7, -640.0f/480.0f)
    ...
    //pangolin::GlTexture glTex0, glTex1, glTex2, glTex3;
    pangolin::GlTexture glTex[NUM_CAM];
    for(; !pangolin::ShouldQuit(); nFrame++)
    {
	//pb::ImageArray vImgs0, vImgs1, vImgs2, vImgs3;
	std::shared_ptr<pb::ImageArray> vImgs = pb::ImageArray::Create();
	...
	//		    nImgWidth, nImgHeight, vImgs[ii].Format(), true, 0,
	//		    vImgs[ii].Format(),vImgs[ii].Type(), 0
	//		    );
		}

		cameraView[ii].Activate();
		//glTex[ii].Upload( vImgs[ii].data(), vImgs[ii].Format(), vImgs[ii].Type() );
