...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
OpenNIDriver::OpenNIDriver( const Uri& uri )
{
  ...
  CameraDriverInterface::SetDefaultProperties({
      ...
      {"baseline", "0.1", "Steve McGuire please explain..."},
      {"depthFocalLength", "0.1", "Steve McGuire please explain..."}
      });
  if( !CameraDriverInterface::ParseUriProperties( uri.properties ) ){
    std::cerr << "OpenNIDriver knows about the following properties:\n";
    CameraDriverInterface::PrintPropertyMap();
    return;
  }

  ImageDim Dims       = GetProperty("size", ImageDim(640,480));
  unsigned int nFPS   = GetProperty("fps", 30);
  bool bCaptureRGB    = GetProperty("rgb", true);
  bool bCaptureDepth  = GetProperty("depth", true);
  bool bCaptureIR     = GetProperty("ir", false);
  bool bAlignDepth    = GetProperty("align", false);
  std::string scmod   = GetProperty<std::string>("cmod", "");
  ...
  m_nImgWidth = Dims.x;
  m_nImgHeight = Dims.y;
  MapMode.nXRes = Dims.x;
  MapMode.nYRes = Dims.y;
