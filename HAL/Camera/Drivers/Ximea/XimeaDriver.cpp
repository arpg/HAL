...
///////////////////////////////////////////////////////////////////////////
XimeaDriver::XimeaDriver( PropertyMap& default_properties )
: device_properties_(default_properties)
{
  float fps        = device_properties_.GetProperty<float>("fps", 0);
  int exp          = device_properties_.GetProperty<int>("exp", 0);
  float gain       = device_properties_.GetProperty<float>("gain", 0.5);
  std::string mode = device_properties_.GetProperty<std::string>("mode", "MONO8");
  ...
  int sync         = device_properties_.GetProperty<int>("sync", 0);
  uint8_t binning  = device_properties_.GetProperty<uint8_t>("binning", 0);
  uint8_t bus_cams = device_properties_.GetProperty<uint8_t>("bus_cams", 0);
  ...
  while(true) {
    ...
    vector_ids.push_back(device_properties_.GetProperty<std::string>(key, ""));
  }
