        ...
        )
{
    for(size_t r = 0; r < cam_to.Height(); ++r) {
        for(size_t c = 0; c < cam_to.Width(); ++c) {
            const Eigen::Vector3d p_o = H_on * Eigen::Vector3d(r,c,1);
            
            // Remap
            Eigen::Vector2d p_warped = cam_to.Map(cam_from.Unmap(
        ...
        )
{
    ...
    for(int r = 0; r < lookup_warp.rows(); ++r) {
        for(int c = 0; c < lookup_warp.cols(); ++c) {
            ...
            mapx.at<float>(r,c) = p[0];
            mapy.at<float>(r,c) = p[1];
        }
    }
}
...
{
    const bool success = m_input->Capture( vImages );

    if(success) {
        pb::Image img[2] = { vImages.mutable_image(0), vImages.mutable_image(1)};
        
        for(int k=0; k < 2; ++k) {
            cv::remap( (cv::Mat)img[k], rimg[k], rmap[k][0], rmap[k][1], CV_INTER_LINEAR);
        }
        cv::imshow("test0", rimg[0]);
        cv::imshow("test1", rimg[1]);
        cv::waitKey(1);
    }
