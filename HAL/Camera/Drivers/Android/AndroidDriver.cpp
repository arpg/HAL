...
{    
    ...
    const android::String8 params8(camParamsString.c_str(),camParamsString.length());
    if( m_Cam->setParameters(params8) != android::NO_ERROR ) {
        LOGE("Failed to set params for camera.\n");
        throw hal::DeviceException("Failed to set params for camera.");
    }
