...
namespace hal 
{
	FileReaderDriver::FileReaderDriver( PropertyMap& device_properties )
		: m_bShouldRun(false), m_nFramesProcessed(0),
			device_properties_(device_properties)
  {
    m_nCurrentImageIndex = device_properties_.GetProperty("startframe", 0);
    m_bLoop              = device_properties_.GetProperty("loop", false);
    m_nBufferSize        = device_properties_.GetProperty("buffer", 10);
    m_sName              = device_properties_.GetProperty("name", std::string("FileCam"));
    m_sId                = device_properties_.GetProperty("id", std::string());
    frequency_           = device_properties_.GetProperty("frequency", 30.0);
    bool Grey            = device_properties_.GetProperty("grey", false);
    m_iCvImageReadFlags  = Grey ? 0 : -1;
		std::string url = device_properties_.GetProperty("url", "");
    regexs_              = Expand(url, '[', ']', ',');

    for( std::string& s : regexs_ ) {
      s = ExpandTildePath(s);
    }
    m_nNumChannels = regexs_.size();
    ...
    for(unsigned int ii = 0; ii < m_nNumChannels; ii++) {
      ...
      if(!hal::WildcardFileList(regexs_[ii], vFiles)) {
        std::cerr << "No files found from regexp '" << regexs_[ii] << "'.\n";
        return;
      }
    }
