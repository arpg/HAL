...
namespace hal 
{
  ProtoReaderDriver::ProtoReaderDriver(
      ...
      m_first(true),
      m_camId(uri.properties.GetProperty<int>("id",-1)),
      m_reader(hal::Reader::Instance(ExpandTildePath(uri.url),hal::Msg_Type_Camera))
  {
    ...
    if( !CameraDriverInterface::ParseUriProperties( uri.properties ) ){
      std::cerr << "ProtoReaderDriver knows about the following properties:\n";
      CameraDriverInterface::PrintPropertyMap();
      return;
    }

    size_t startframe  = GetProperty("startframe", 0);
    m_reader.SetInitialImage(startframe);
